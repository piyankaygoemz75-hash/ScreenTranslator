...
先安装并运行屏译桌面程序。桌面程序会自动注册 Native Messaging Host，并在“常规 → 网页译文跟随”中提供 Chrome/Edge 安装引导。只加载扩展而没有运行桌面程序时，扩展会在后台限速重连，不影响网页使用。

### Chrome

1. 在屏译中点击“安装到 Chrome”，程序会打开 `chrome://extensions`、打开扩展目录并复制目录路径。
2. 开启右上角的“开发者模式”。
3. 点击“加载已解压的扩展程序”。
...
1. 在屏译中点击“安装到 Edge”，程序会打开 `edge://extensions`、打开扩展目录并复制目录路径。
2. 开启左侧的“开发人员模式”。
3. 点击“加载解压缩的扩展”。
4. 选择发布目录中的 `browser-extension` 文件夹。
5. 确认扩展 ID 是 `plpgmkbadcfnkmolbeecggbbopilajed`。
...
- 支持页面根滚动以及普通嵌套滚动容器。
- 支持 X/Twitter 等单页应用在站内路由切换后恢复视口状态并继续跟随。
